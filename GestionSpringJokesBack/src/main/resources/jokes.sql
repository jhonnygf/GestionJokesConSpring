...
--
-- 1) Crear la tabla "primera_vez"
CREATE TABLE primera_vez (
  id SERIAL PRIMARY KEY,
  programa        VARCHAR(100)  NOT NULL,
  fecha_emision   DATE          NOT NULL,
  -- Relación 1:1 con jokes
  id_joke  INTEGER UNIQUE,           
  CONSTRAINT fk_joke_primera 
    FOREIGN KEY (id_joke) 
    REFERENCES jokes(id) 
    ON DELETE CASCADE
);
...
-- 2) Crear la tabla "telefonos"
CREATE TABLE telefonos (
  id         SERIAL     PRIMARY KEY,
  numero     VARCHAR(20)   NOT NULL,
  id_primera INTEGER        NOT NULL,
  CONSTRAINT fk_primera
    FOREIGN KEY (id_primera)
    REFERENCES primera_vez (id)
    ON DELETE CASCADE
);
